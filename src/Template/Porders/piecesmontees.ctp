<div class="block pad form large-12 medium-12 left">
    ...
    <fieldset>
        ...
        <?php 
            echo $this->Form->label('costumer', 'Client');
            echo $this->Form->text('costumer', ['style'=>'width:15% !important; max-width:150px','required'=>true,'onchange'=>'getClientName(this.value);']) ;
            echo "&nbsp;&nbsp;&nbsp;";
            echo $this->Form->text('costumer_name', ['style'=>'width:45% !important;','id'=>'costumer_name','disabled'=>true]) ?>
        


        <?php echo $this->Form->input('nbPiece', ['type'=>'number','min'=>1, 'label'=>'Nombre de pièces voulues','empty'=>'Nombre de pièces voulues','style'=>'width:150px !important;','onchange'=>'addRows(this.value);']); ?>
...
<script type="text/javascript">
    ...
    function getEntremets(biscuitId,obj){ 
        ...
        var d;
        var table = [];
        $.get( "/entremets/listerajax", { biscuitId:biscuitId } )
            .done(function( data ) {
                // alert(data);
            d =  '{"Entremets":' + data + '}';
            // alert(d);
    ...
    function getClientName(id){ 
        // http://localhost/cakeaddict
        $.get( "/costumers/findid", { id:id } )
            .done(function( data ) {
                document.getElementById('costumer_name').value = data;
