...
<div class="biscuitEntremets form large-9 medium-9 columns">
    <?= $this->Form->create($biscuitEntremet) ?>
    <fieldset>
        <br>
        <legend><?= __('Association Biscuits et Entremets') ?></legend>
        <br>
        <?php  echo $this->Form->input('biscuit_id',['label'=>'Biscuit','type'=>'select','options'=>$biscuits]);      ?>
        <?php  echo $this->Form->input('entremet_id',['label'=>'Entremet','type'=>'select','options'=>$entremets]);      ?>
        <br>
